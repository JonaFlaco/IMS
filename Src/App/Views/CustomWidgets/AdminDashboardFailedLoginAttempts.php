...
foreach($data as $itm){
    ...
    <div class="d-flex align-items-start mt-1 mb-1">
        ...
        <div class="w-100 overflow-hidden">
            <span class="float-end"><?= \App\Helpers\DateHelper::humanify( strtotime($itm->last_try)); ?></span>    
        
            <a class="text-dark" target="_blank" href="/users/show/<?= $itm->id; ?>">
                <h5 class="mt-0 mb-1"><?= $itm->full_name ?></h5>
            </a>
            <span class="font-13">
                <span class="badge bg-<?= ($itm->is_active ? "success" : "danger") ?>"><?= ($itm->is_active ? "Active" : "Disabled") ?></span>
                &nbsp; &nbsp;
                <?= $itm->no_of_tries ?> failed attempts 
            </span>
        </div>
    </div>
