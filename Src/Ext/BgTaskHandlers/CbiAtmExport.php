...
class CbiAtmExport extends BgTaskHandlers
{
    ...
    public function GenerateOneReport($ids_array, $spreadsheet)
    {
        ...
        foreach ($ids_array as $id) {
            $casesAtm = $this->coreModel->nodeModel("b_services")
                ->where("m.id = :ids and m.sub_service in( :atm ,:regularizacion) ")
                ->bindValue("ids", $id)
                ->bindValue("atm", 26)
                ->bindValue("regularizacion", 8)
                ->load();
            $allCasesAtm = array_merge($allCasesAtm, $casesAtm);
        }
        ...
        foreach ($allCasesAtm as $item) {
            ...
            $sheet->setCellValue('P' . $row, $item->sub_service_display);
        ...
        $filename = UPLOAD_DIR_FULL . DS . "bg_tasks"  . DS . 'Cash ATM reporte' .  time() . '.xlsx';
